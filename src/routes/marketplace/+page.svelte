...
<div class="w-full p-3 pb-0">
	<Input value={name} placeholder="Search plugin..."/>
</div>
<ScrollArea>
	<div class="w-full h-full grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-3 gap-3">
		{#each PluginManager.plugins.filter(p => p.name.toLowerCase().includes(name.toLowerCase())).sort((a, b) => a.name.localeCompare(b.name)) as plugin}
			<Plugin {plugin}/>
		{/each}
	</div>
</ScrollArea>
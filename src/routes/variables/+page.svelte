            ...
            <Collapsible.Root>
                <div class="flex w-full text-sm bg-card pl-5 pr-2 py-1 border-b-1">
                    <div class="flex gap-7 w-full">
                        <label class="mt-auto mb-auto w-fit font-semibold">{name}</label>
                        {#if typeof debugVariables.get(name) !== "object" || debugVariables.get(name) == null}
                            <label class="mt-auto mb-auto ml-auto overflow-hidden text-ellipsis">{debugVariables.get(name) ?? String(debugVariables.get(name))}</label>
                        {/if}
                    </div>
    ...
    {#snippet variable(name, parent, prefix)}
        <Collapsible.Root>
            <div class="flex w-full text-xs bg-card pl-5 pr-2 py-1 border-b-1">
                <div class="flex gap-7 w-full">
                    <label class="mt-auto mb-auto w-fit">{prefix}{name}</label>
                    {#if typeof parent[name] !== "object" || parent[name] == null}
                        <label class="mt-auto mb-auto ml-auto overflow-hidden text-ellipsis">{parent[name] ?? String(parent[name])}</label>
                    {/if}
                </div>

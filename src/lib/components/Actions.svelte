    ...
    <Card.Content class="p-1 pt-0 px-0 pb-0 h-full overflow-hidden border-0">
        <div class="flex h-fit pr-0 px-1.5 py-0.5 font-bold border-1">
            ...
            <Button variant="ghost" size="icon"
                    class="!p-1 !w-fit !h-fit !bg-transparent !cursor-pointer {!App.selectedTrigger ? 'hidden' : ''}"
                    onclick={() => {isCreatingAction = true; pos = {x: 0, y: 0}}}>
                <PlusIcon/>
            </Button>
            <Button variant="ghost" size="icon"
                    class="!p-1 !w-fit !h-fit !bg-transparent !cursor-pointer {!App.selectedTrigger ? 'hidden' : ''}"
        ...
        <div class="bg-muted h-full w-full">
            {#if App.selectedTrigger}
                <SvelteFlow onbeforedelete={({nodes, edges}) => {
                    BotManager.selectedBot.markAsModified(App.selectedTrigger.id)
                    if(document.activeElement.tagName !== "BODY" && !document.activeElement.getAttribute("class").startsWith("svelte-flow") || getSelection().toString()) return false;
                        ...
                        App.updateUndo()
                        return {nodes: nodes.filter(n => n.actionType), edges}
                    }
                ...
                }} onnodedragstart={() => App.updateUndo()} onconnectstart={() => App.updateUndo()}
                            proOptions={{ hideAttribution: true }} colorMode="dark" edgeTypes={{default: Edge}}
                            nodeTypes={{action: Node, group: Group}} bind:nodes={selectedTrigger.actions}
                            bind:edges={selectedTrigger.edges}>
...
<Modal bind:open={isCreatingAction} title="Add Action" onDone={() => addAction()}>
    <div class="grid gap-4 py-4 px-1">
        <div class="grid grid-cols-4 items-center gap-4">
            ...
            {#await Promise.all((BotManager.selectedBot?.actionClasses ?? []).map(el => el.type).sort().map(async el => ({
                label: await App.translate(el, App.selectedLanguage),
                value: el
            }))) then actions}
                {#await App.translate("Group", App.selectedLanguage) then group}
                    {#await App.translate("None", App.selectedLanguage) then none}
                        <SearchSelect name="type"
                                      values={[{label:none, value: "None", disabled:true}, {label: group, value:"group"}, ...actions]}
                                      bind:value={actionType}
                                      class="col-span-3 w-full {actionType === 'None' ? 'ring-2 ring-destructive' : ''}"/>
                    {/await}
                {/await}
            {/await}
        </div>
...
<Modal bind:open={App.isEditingAction}
       title="{App.selectedTrigger?.actions?.filter(act => act.type !== 'group').findIndex(act => act.id === App.selectedAction?.id)}. {App.selectedAction?.actionType}"
       onDone={() => editAction()}
       url="https://discord-bot-engine.gitbook.io/discord-bot-engine/actions/{App.selectedAction?.actionType.toLowerCase().replaceAll(' ', '-')}">

            ...
            <Sidebar.GroupContent>
                <Sidebar.Menu>
                    {#each items as item, i (i)}
                        <Sidebar.MenuItem>
                            <Sidebar.MenuButton
                                    class={page.url.pathname === item.url ? "!bg-primary !text-primary-foreground" : ""}>
                                {#snippet child({props})}
                                    <Translation text={item.title} el={itemEl}/>
                                    {#snippet itemEl(text)}
                                        <button disabled={!App.hasNode || !App.hasPython}
                                                title={text}
                            ...
                            <DropdownMenu.Content side="right">
                                <DropdownMenu.Group>
                                    <ScrollArea class="max-h-[50vh] [&>div]:max-h-[50vh]">
                                        <DropdownMenu.Item class="!text-xs" onclick={() => setTheme(null)}>
                                            <CheckIcon
                                        ...
                                        {#each App.themes as theme}
                                            <DropdownMenu.Item class="!text-xs" onclick={() => setTheme(theme)}>
                                                <CheckIcon
                                                ...
                                                {#if !App.hasNode || !App.hasPython}
                                                <Sidebar.MenuItem>
                                                <Sidebar.MenuButton>
                                                {#snippet child({ props })}
                                                <Translation text={"Missing Requirements"} el={errorEl}/>
                                                {#snippet errorEl(text)}
                                                <a
                                                title={text}
                                                onclick={() => App.showWarning = true}
                                                {...props}
                                                style="padding:0!important">
                                                <ErrorIcon class="relative w-5.5 h-5.5 right-0 m-auto"/>
                                                </a>
                                                {/snippet}
                                                    {/snippet}
                                                </Sidebar.MenuButton>

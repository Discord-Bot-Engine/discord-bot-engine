...
<div class="flex flex-col bg-card rounded-none shadow-sm">
    <Button variant="ghost" class="w-full rounded-none border-1 h-fit !p-0" onclick={() => App.hideConsole = !App.hideConsole}>
        {#if !App.hideConsole}
            <ChevronDownIcon/>
        {:else}
            <ChevronUpIcon/>
    ...
    {#if !App.hideConsole}
<Card.Root class="!rounded-none border-t-0 w-full h-[30vh] min-h-50 p-1 relative shadow-none">
    <Card.Content class="p-1 px-1 overflow-hidden h-full flex flex-col">
        ...
        <ScrollArea class="text-sm flex-1 overflow-hidden">

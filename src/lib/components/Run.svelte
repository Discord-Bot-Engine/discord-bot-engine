...
<Card.Root class="w-full h-full p-1 relative">
    <Card.Content class="p-1 overflow-hidden h-full">
        <div class="flex justify-end h-fit">
            ...
            <Button variant={BotManager.selectedBot?.isRunning ? undefined : 'outline'} size="icon" class="size-8 rounded-lg rounded-r-none" onclick={() => BotManager.runBot()}><PlayIcon size={16} /></Button>
            <Button variant={BotManager.selectedBot?.isRunning ? 'outline' : undefined} size="icon" class="size-8 rounded-lg rounded-l-none" onclick={() => BotManager.stopBot()}><SquareIcon size={16} /></Button>
        </div>

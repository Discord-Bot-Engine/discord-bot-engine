...
class BotManagerClass {
    ...
    installPackages(path) {
        return new Promise(async (resolve) => {
            ...
            invoke('install_bot_packages', {bot_path: path});
        })
    }
    ...
    updateBotFiles() {
        return new Promise(async (resolve) => {
            ...
            const unlisten = await listen('finished_copying', async () => {
                ...
                await this.installPackages(this.selectedBot.path)
    ...
    async saveBotData() {
        ...
        await invoke('save_bot_triggers', {
            bot_path: this.selectedBot.path,
            modified_triggers: this.selectedBot.modifiedTriggers,
            trigger_contents: triggerContents,
            removed_triggers: this.selectedBot.removedTriggers
        })
        await invoke('save_bot_extensions', {
            bot_path: this.selectedBot.path,
    ...
    async uploadBotFiles() {
        this.selectedBot.isLoading = "Connecting"

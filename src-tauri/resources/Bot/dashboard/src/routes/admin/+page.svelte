<script>
	...
	let name = $state("")
	let value = $state("")
	let types = ["Text", "Dropdown", "Member", "Channel", "Role", "Mentionable"].sort()
...
<div class="w-full h-full p-3">
	<div class="flex">
		<h1 class="font-bold text-xl">Editing {data.botName}</h1>
				...
				<div class="grid gap-4 py-4">
					...
					{#if type === "Dropdown" || type === "Text"}
						<div class="grid grid-cols-4 items-center gap-4">
							...
							<Input id="value" bind:value={value} class="col-span-3" />
						</div>
						{/if}
					{#if type === "Dropdown"}
						<div class="grid grid-cols-4 items-center gap-4">
							<Label for="value" class="text-right">Values</Label>
							<Input id="value" bind:value={values} placeholder="Separated by ," class="col-span-3" />
						</div>
					{/if}
				</div>
				<Dialog.Footer>
